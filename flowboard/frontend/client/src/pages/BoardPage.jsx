...
import AiInsightsModal   from '../components/board/AiInsightsModal';
...
export default function BoardPage() {
  ...
  const [aiTask,    setAiTask]    = useState(null);   // task object for AI insights
  const [deleteId,  setDeleteId]  = useState(null);   // task id to confirm delete
  const [creating,  setCreating]  = useState(false);
  const [updating,  setUpdating]  = useState(false);

  const resolvedEditTask = useMemo(() => {
    if (!editTask) return null;
    return tasks.find((t) => t._id === editTask._id) ?? editTask;
  }, [editTask, tasks]);

  const resolvedAiTask = useMemo(() => {
    if (!aiTask) return null;
    return tasks.find((t) => t._id === aiTask._id) ?? aiTask;
  }, [aiTask, tasks]);
