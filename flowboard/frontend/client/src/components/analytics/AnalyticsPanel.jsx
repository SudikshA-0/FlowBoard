...
import { getDelayRiskPercent, getDelayRiskLabel } from '../../services/mlApi';
...
export default function AnalyticsPanel() {
  ...
  // ── ML Insights (risk counts; completed tasks excluded) ─────────────────
  const mlCounts = tasks.reduce((acc, t) => {
    if (t.status === 'done') return acc;
    const pct = getDelayRiskPercent(t);
    const label = getDelayRiskLabel(pct);
    if (label === 'High') acc.high += 1;
    else if (label === 'Medium') acc.medium += 1;
    else if (label === 'Low') acc.low += 1;
    return acc;
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 flex flex-col gap-6">
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="font-bold text-sm text-slate-700 dark:text-slate-300">ML Insights</p>
          <span className="text-[11px] text-slate-400">Delay risk distribution</span>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 rounded-2xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20">
            <div className="text-[10px] font-bold text-red-600 dark:text-red-400">High</div>
            <div className="text-xl font-bold text-slate-800 dark:text-slate-100">{mlCounts.high}</div>
          </div>
          <div className="p-3 rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20">
            <div className="text-[10px] font-bold text-amber-600 dark:text-amber-400">Medium</div>
            <div className="text-xl font-bold text-slate-800 dark:text-slate-100">{mlCounts.medium}</div>
          </div>
          <div className="p-3 rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20">
            <div className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">Low</div>
            <div className="text-xl font-bold text-slate-800 dark:text-slate-100">{mlCounts.low}</div>
          </div>
        </div>
        <p className="text-[11px] text-slate-400 mt-3">
          Active tasks only (excludes Done). ML API: <span className="font-mono">/predict</span> on port 5000.
        </p>
      </div>
    </div>
  );
