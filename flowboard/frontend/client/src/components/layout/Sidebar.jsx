...
import { Filter, Flag, CheckCircle2, Clock, AlertCircle, LayoutGrid, Folder, Users, Plus, Link2, Trash2 } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const { loading: projectsLoading, personalProjects, teams, teamProjects, activeProjectId, setActiveProject, getInviteLink, deleteProject, deleteTeam } = useProjects();
      ...
      <div className="card p-4">
        ...
        <div className="mb-2">
          ...
          {(personalProjects ?? []).map((p) => {
            ...
            return (
              <div key={p._id} className="flex items-center gap-1 mb-1">
                <button
                  onClick={() => setActiveProject(p._id)}
                  className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all text-left
                    ${active
                ...
                {canDelete && (
                  <button
                    ...
                    onClick={async (e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      const ok = window.confirm(`Delete project \"${p.name}\"? This will remove its tasks for you.`);
                      if (!ok) return;
                      try {
        ...
        <div>
          ...
          {(teams ?? []).map((t) => {
            ...
            const isAdmin = !!user?._id && t.adminId === user._id;
            return (
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {isAdmin && (
                      <button
                        ...
                        title="Delete team"
                        onClick={async () => {
                          const ok = window.confirm(`Delete team \"${t.name}\"? This will remove team projects for you.`);
                          if (!ok) return;
                          try {
                  ...
                  projects.map((p) => {
                    ...
                    return (
                      <div key={p._id} className="flex items-center gap-1 mb-1">
                        <button
                          onClick={() => setActiveProject(p._id)}
                          className={`flex-1 flex items-center gap-2 pl-5 pr-3 py-2 rounded-xl text-sm font-medium transition-all text-left
                            ${active
                              ? 'bg-brand-50 dark:bg-white/5 text-brand-600 dark:text-brand-300'
                              : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5'}`}
                        >
