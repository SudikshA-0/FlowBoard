...
export default function TaskForm({ initial, onSubmit, onCancel, submitLabel = 'Create Task' }) {
  ...
  useEffect(() => {
    // Hydrate when opening a new task OR when the same task is updated.
    setForm(normalizeInitial(initial));
    // Reset transient UI state on task switch.
    setTagIn('');
    setErrors({});
