...
import { Calendar, Brain, Clock, Trash2, Pencil } from 'lucide-react';
import { format, isPast, isToday } from 'date-fns';
import PriorityBadge from '../ui/PriorityBadge';
import { getDelayRiskDisplay } from '../../services/mlApi';

export default function TaskCard({ task, index, onEdit, onAi, onDelete }) {
  ...
  const { pct, label, isDone } = getDelayRiskDisplay(task);
  const badgeText = pct == null ? null : (isDone ? `Delay Risk: ${pct}% (Completed)` : `Delay Risk: ${pct}%`);
    ...
    <Draggable draggableId={task._id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          ...
          <div className={`absolute top-2.5 right-2.5 flex gap-2 transition-opacity duration-150
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onAi?.(task); }}
              className="w-6 h-6 rounded-lg bg-white dark:bg-slate-700 border border-slate-200
                         dark:border-slate-600 flex items-center justify-center text-slate-500
                         hover:text-violet-500 hover:border-violet-300 transition-colors shadow-sm"
              title="AI Insights"
            >
              <Brain size={12} />
            </button>
          ...
          {/* Top row: priority + ML risk */}
          <div className="mb-2 flex items-center justify-between gap-2 pr-12">
            <PriorityBadge priority={task.priority} />
            {pct != null && (
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full border text-[10px] font-bold ${badgeStyle}`}>
                {isDone ? badgeText : `${badgeText} (${label})`}
              </span>
            )}
          </div>
